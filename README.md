...
加分项：提交数据异步提交不刷新页面（ajax技术），使用框架，使用连接池（我选择了阿里的Druid 具体如何配置百度即可），项目部署到远程服务器（Ubuntu），进行操作时有确认提示
...
首先在`index.jsp`页面中，通过输入表单的数据，在点击提交按钮后触发`insert.js`的校验策略(但是删除的表单则是在输入之后，就会进行校验)，不同的表单对应于不同的策略。总共三个表单：User表单使用user_check()，Person表单使用person_check()，Delete使用del_check()，前两者都是之间校验数据是否符合规定，使用了jQuery进行校验，而对于最后一个，则是将数据发送到servlet，来查看当前的用户名是否存在，依旧使用了BootstrapValidator。

如果校验通过，那么就执行`submit.js`中相应的表单提交的操作。通过jQuery将对应按钮的id的click与相关的函数进行绑定，从而为不同的按钮定义不同的行为。在user和person进行提交时，会先向对应的servlet发出一个请求，其type为'search'，用来检查此次提交操作是否会修改已存在的值，并且按照不同的逻辑进行回馈。如果确定要覆盖掉之前的值或者是插入了一个新值，在经过交互处理之后，会再次向servlet发出请求，并且根据是否成功在当前页面返回相应的值。

#### 其他的页面`result.jsp`和`show_data.jsp`

此时整体的实验已经完成，还剩下最后的一个查看数据库数据的页面：`show_data.jsp`。这一个页面的处理使用了jstl。通过在`index.jsp`点击herf链接后，跳转到对应的servlet：`queryServlet`，该servlet中，进行数据库user表和person表的查询操作，并将得到的两个list作为request的参数，一同发送到`show_data.jsp`。

在`show_data.jsp`中，则是通过jstl遍历得到的两个list，将数据依次取出显示在表格中。

而`result.jsp`页面则是在每次进行操作时，在对应的servlet内，将操作的行为和结果放入到session中，在页面中也是使用了jstl从session中取出数据，进行显示。这里在servlet内放入到session中时，使用了数字的形式来代表每一个操作，在页面中解析并进行对应的显示。
